...
//GoodDungeon class inherits from the Dungeon class
class GoodDungeon : Dungeon
{
    //Private field to store the minimum room size
    private int _minimumRoomSize;

    //Constructor for GoodDungeon, calls base constructor with size parameter
    ...
    protected override void generate(int pMinimumRoomSize)
    {
        ...
        //Create the main room with the full dungeon size
        Room mainRoom = new Room(new Rectangle(0, 0, size.Width, size.Height));

        //Split the main room into smaller rooms
        Split(mainRoom);

        //Log the number of rooms created
        Console.WriteLine(rooms.Count + " rooms created.");

        //Remove the largest and smallest rooms
        RemoveRooms();
        Console.WriteLine("Removing biggest and smallest doors.");
    ...
    //Method to remove the largest and smallest rooms from the list
    void RemoveRooms()
    {
        int biggestRoomArea = 0;
        int smallestRoomArea = int.MaxValue;

        // Find the largest and smallest room by surface area
        foreach (Room room in rooms)
        {
            if (room.surfaceArea > biggestRoomArea)
            {
                biggestRoomArea = room.surfaceArea;
            }

            if (room.surfaceArea < smallestRoomArea)
            {
                smallestRoomArea = room.surfaceArea;
            }
        }

        // Log the size of the largest and smallest rooms
        Console.WriteLine("Biggest room: " + biggestRoomArea);
        Console.WriteLine("Smallest room: " + smallestRoomArea);

        // Create a new list to hold the rooms that need to be removed
        List<Room> roomsToRemove = new List<Room>();

        // Identify all rooms with the biggest and smallest surface areas
        foreach (Room room in rooms)
        {
            if (room.surfaceArea == biggestRoomArea || room.surfaceArea == smallestRoomArea)
            {
                roomsToRemove.Add(room);
            }
        }

        // Remove the identified rooms from the original list
        foreach (Room room in roomsToRemove)
        {
            rooms.Remove(room);
            Console.WriteLine("Biggest/smallest room removed.");
        }
    }

    //Method to place doors connecting adjacent rooms
